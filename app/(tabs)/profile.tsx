...
import { StyleSheet, View, ScrollView, TouchableOpacity, Image, Switch as RNSwitch, Platform, ActivityIndicator } from 'react-native';
...
const MENU_ITEMS = [
  ...
  { id: 'points', title: 'Spice Points', icon: 'ribbon-outline', extra: '0 pts', route: '/points' },
  { id: 'notifications', title: 'Notifications', icon: 'notifications-outline', route: '/notifications' },
  { id: 'admin', title: 'Admin Dashboard', icon: 'shield-checkmark-outline', route: '/admin' },
...
export default function ProfileScreen() {
  ...
  const renderUserCard = () => {
      ...
      return (
        <View style={[styles.userCard, { backgroundColor: colors.card || '#fff', justifyContent: 'center', paddingVertical: 40 }]}>
          <View style={{ alignItems: 'center' }}>
            ...
            <Button title="LOGIN / SIGN UP" onPress={() => router.replace('/(auth)/login')} style={{ width: '100%' }} />
          </View>
        </View>
      );
    }
    ...
    const displayName = profile?.full_name || user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Spice Enthusiast';
      ...
      <View style={[styles.userCard, { backgroundColor: colors.card || '#fff' }]}>
        ...
        <View style={styles.userInfo}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <View style={{ flex: 1 }}>
              <Text variant="h2" family="heading" numberOfLines={1}>{displayName}</Text>
              <Text variant="caption" style={{ opacity: 0.6 }} numberOfLines={1}>{displayEmail}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  editBtn: { padding: 8, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10 },
  tierBadge: {
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 6,
