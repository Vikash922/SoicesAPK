...
export function SpiceHeatMeter({ level, showLabel = true, animated = true }: SpiceHeatMeterProps) {
  const animatedLevel = useSharedValue(animated ? 0 : level / 10);

  useEffect(() => {
    if (animated) {
      animatedLevel.value = withTiming(level / 10, { duration: 1000 });
    } else {
      animatedLevel.value = level / 10;
    }
  }, [level, animated]);
